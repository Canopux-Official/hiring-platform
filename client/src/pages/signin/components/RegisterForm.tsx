...
interface Props {
  ...
  onBack: () => void;
  onField: (key: keyof FormState) => (e: React.ChangeEvent<HTMLInputElement>) => void;
  onSubmit: (e: React.FormEvent) => Promise<{ pendingApproval?: boolean } | void>; // ✅ updated
  onGoSignIn: () => void;
}
...
export default function RegisterForm({
  ...
  // ✅ Pending approval screen — shown after successful recruiter registration
  if (pendingApproval) {
    return (
      <motion.div
        key="pending"
      ...
      >
        <Card sx={{ p: { xs: 4, md: 6 }, maxWidth: 520, mx: "auto", textAlign: "center" }}>
          <Box sx={{
            ...
            <HourglassTopIcon sx={{ fontSize: 36, color: "#34d39e" }} />
          </Box>
  ...
  return (
    ...
    >
      <Card sx={{ p: { xs: 4, md: 6 }, maxWidth: 520, mx: "auto" }}>
        ...
        {/* ✅ Info banner for recruiters so they know upfront */}
        {role === "recruiter" && (
