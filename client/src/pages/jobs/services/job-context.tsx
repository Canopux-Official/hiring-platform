...
import { LocalJob, EmploymentType, WorkMode, Urgency } from "../types";

// ─── Storage Key ──────────────────────────────────────────────────────────────

const STORAGE_KEY = "Rozgaari.jobs.v1";

// ─── Seed Data ────────────────────────────────────────────────────────────────

const seedJobs: LocalJob[] = [
  {
    id: "seed-1",
    ...
    salaryMax: 210000,
    experience: "6+ years",
    skills: ["Figma", "Design Systems", "Prototyping", "AI/UX"],
  ...
  },
  {
    ...
    salaryMax: 145000,
    experience: "7+ years",
    skills: ["Rust", "Tokio", "Distributed Systems", "PostgreSQL"],
  ...
  },
  {
    ...
    salaryMax: 300000,
    experience: "10+ years",
    skills: ["Luxury Hospitality", "Team Leadership", "VIP Relations"],
    ...
    urgency: "Standard",
    screening: [],
    postedAt: Date.now() - 2 * 86400 * 1000,
    match: 81,
    status: "published",
  },
  {
    id: "seed-4",
    title: "Delivery Fleet Lead",
    company: "RapidMile",
    logo: "R",
    category: "Logistics",
    employmentType: "Full-time",
    ...
    salaryMax: 950000,
    experience: "4+ years",
    skills: ["Fleet Management", "Last-mile", "Operations"],
  ...
  },
  {
    ...
    salaryMax: 120000,
    experience: "8+ years",
    skills: ["Brand Strategy", "Art Direction", "Campaign"],
...
export function JobsProvider({ children }: { children: React.ReactNode }) {
  const [jobs, setJobs] = useState<LocalJob[]>(seedJobs);
  ...
  const addJob = useCallback(
    (j: LocalJob) => setJobs((prev) => [j, ...prev]),
    []
  );

  const updateJob = useCallback(
    (id: string, patch: Partial<LocalJob>) =>
      setJobs((prev) =>
  ...
  const value = useMemo(
    () => ({ jobs, addJob, updateJob, deleteJob, duplicateJob }),
    [jobs, addJob, updateJob, deleteJob, duplicateJob]
  );
...
export function formatLocalSalary(
  j: Pick<LocalJob, "currency" | "salaryMin" | "salaryMax">
) {
  const sym: Record<string, string> = {
    USD: "$", EUR: "€", GBP: "£", INR: "₹",
    AED: "AED ", CAD: "CA$", JPY: "¥", AUD: "A$", SGD: "S$",
  };
