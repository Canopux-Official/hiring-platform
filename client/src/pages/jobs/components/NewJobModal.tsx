...
import { createJob, CreateJobPayload } from "../../RecruiterDashboard/services/recruiter";
import { useToast } from "../../../hooks/useToast";
import { JobType, ExperienceLevel, NewJobFormState, EmploymentType, WorkMode, Urgency } from "../types";
...
export default function NewJobModal({ open, onClose, onSuccess }: NewJobModalProps) {
  ...
  const update = <K extends keyof NewJobFormState>(k: K, v: NewJobFormState[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
    ...
    <Dialog
      ...
      PaperProps={{ 
        sx: { 
          ...
          borderRadius: 4,
          boxShadow: "0 25px 70px -15px rgba(15, 23, 42, 0.15)"
        } 
      }}
    >
      {/* Title */}
      ...
      <LinearProgress 
        variant="determinate" 
        value={progress} 
        sx={{ height: 4, bgcolor: "#e2e8f0", "& .MuiLinearProgress-bar": { bgcolor: GREEN } }} 
      />
      ...
      <DialogContent sx={{ pt: 4 }}>
        ...
        <Grid container spacing={5}>
          <Grid item xs={12} md={7}>
            ...
            {step === 0 && (
              <Stack spacing={3}>
                ...
                <TextField label="Company Name" value={form.company} onChange={(e) => update("company", e.target.value)} fullWidth />
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={6}>
                    <TextField select label="Employment Type" value={form.employmentType} onChange={(e) => update("employmentType", e.target.value as EmploymentType)} fullWidth>
            ...
            {step === 1 && (
              <Stack spacing={3}>
                <Grid container spacing={2}>
                  <Grid item xs={4}>
                    <TextField select label="Currency" value={form.currency} onChange={(e) => update("currency", e.target.value)} fullWidth>
                      {["INR", "USD", "EUR", "GBP", "AED", "CAD", "AUD", "SGD"].map((c) => <MenuItem key={c} value={c}>{c}</MenuItem>)}
                    </TextField>
                  </Grid>
                  <Grid item xs={4}>
                ...
                <TextField label="Experience Required" value={form.experience} onChange={(e) => update("experience", e.target.value)} fullWidth />
                ...
                <TextField
                  ...
                  helperText={form.description.length > 0 && form.description.length < 50 ? `${50 - form.description.length} more characters needed` : ""}
                />
            ...
            {step === 2 && (
              <Stack spacing={3}>
                <Box>
                  <Typography variant="body2" sx={{ mb: 1, fontWeight: 600 }}>Required Skills</Typography>
                  <Stack direction="row" spacing={1}>
                    <TextField
                      size="small"
                      ...
                      fullWidth
                    />
                    <IconButton onClick={addSkill} sx={{ color: GREEN }}>
                      <AddIcon />
                    </IconButton>
                  </Stack>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 2 }}>
                    {form.skills.map((s) => (
                      <Chip
                        key={s}
                        label={s}
                        onDelete={() => update("skills", form.skills.filter((x) => x !== s))}
                        sx={{ bgcolor: GREEN_LIGHT, color: GREEN }}
                      />
                    ))}
                  </Stack>
                </Box>
                ...
                <TextField label="Benefits & Perks" multiline rows={3} value={form.benefits} onChange={(e) => update("benefits", e.target.value)} fullWidth />
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={4}>
                    <TextField type="date" label="Application Deadline" InputLabelProps={{ shrink: true }} value={form.deadline} onChange={(e) => update("deadline", e.target.value)} fullWidth />
                  </Grid>
                  <Grid item xs={4}>
            ...
            {step === 3 && (
              <Stack spacing={3}>
                <Typography variant="h6" fontWeight={700}>{form.title || "Untitled Role"}</Typography>
                <Typography color="text.secondary">{form.company} • {form.location} • {form.workMode}</Typography>
                <Divider />
                <Typography variant="body1" sx={{ whiteSpace: "pre-line", lineHeight: 1.8 }}>{form.description}</Typography>
                <Typography variant="subtitle2" sx={{ mt: 2 }}>Responsibilities</Typography>
                <Typography variant="body1" sx={{ whiteSpace: "pre-line" }}>{form.responsibilities}</Typography>
                <Typography variant="subtitle2" sx={{ mt: 2 }}>Requirements</Typography>
                <Typography variant="body1" sx={{ whiteSpace: "pre-line" }}>{form.requirements}</Typography>
              </Stack>
            )}
          </Grid>
          ...
          {/* Live Preview */}
          <Grid item xs={12} md={5}>
            <Card sx={{ p: 4, position: "sticky", top: 24, border: `1px solid ${alpha(GREEN, 0.2)}` }}>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 3 }}>
                <Chip size="small" label={form.category} sx={{ bgcolor: GREEN_LIGHT, color: GREEN }} />
                <Chip size="small" label={form.employmentType} variant="outlined" />
                <Chip size="small" label={form.workMode} variant="outlined" />
                <Chip size="small" label={form.urgency} sx={{ bgcolor: alpha(BLUE, 0.1), color: BLUE }} />
              </Stack>

              <Typography variant="h6" sx={{ color: GREEN, fontWeight: 800, mt: 3 }}>
                {form.currency} {Math.floor(form.salaryMin / 1000)}k – {Math.floor(form.salaryMax / 1000)}k
              </Typography>

              {form.skills.length > 0 && (
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 3 }}>
                  {form.skills.slice(0, 6).map((s) => (
                    <Chip key={s} size="small" label={s} sx={{ bgcolor: alpha(GREEN, 0.08), color: GREEN }} />
                  ))}
                </Stack>
              )}
            </Card>
