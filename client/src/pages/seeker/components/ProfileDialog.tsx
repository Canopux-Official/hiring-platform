...
import {
  Dialog, DialogTitle, DialogContent, DialogActions,
  Button, TextField, Stack, IconButton, Checkbox, FormControlLabel,
} from "@mui/material";
import CloseIcon from "@mui/icons-material/Close";
...
export function ExperienceDialog({
  ...
  const validate = (): boolean => {
    ...
    else if (form.title.trim().length < 2) e.title = "Job title must be at least 2 characters.";
    else if (form.title.trim().length > 100) e.title = "Job title cannot exceed 100 characters.";

    if (!form.company.trim()) e.company = "Company name is required.";
    else if (form.company.trim().length < 2) e.company = "Company name must be at least 2 characters.";
    else if (form.company.trim().length > 100) e.company = "Company name cannot exceed 100 characters.";

    if (form.location && form.location.trim().length > 100) e.location = "Location cannot exceed 100 characters.";

    if (!form.startDate) e.startDate = "Start date is required.";

    if (!form.isCurrent) {
      if (!form.endDate) e.endDate = "End date is required unless currently working here.";
      else if (form.startDate && form.endDate && form.endDate < form.startDate)
        e.endDate = "End date cannot be before start date.";
    }
      ...
      <DialogContent>
        <Stack spacing={3} sx={{ pt: 2 }}>
          ...
          <TextField
            ...
            onChange={(e) => { f("description", e.target.value); clearError("description"); }}
            error={!!errors.description}
            helperText={errors.description || `${(form.description || "").length}/1000 characters`}
          />
        </Stack>
      </DialogContent>
...
export function EducationDialog({
  ...
  const validate = (): boolean => {
    ...
    else if (form.degree.trim().length > 100) e.degree = "Degree cannot exceed 100 characters.";

    if (!form.institution.trim()) e.institution = "Institution is required.";
    else if (form.institution.trim().length > 150) e.institution = "Institution cannot exceed 150 characters.";

    if (!form.fieldOfStudy.trim()) e.fieldOfStudy = "Field of study is required.";
    else if (form.fieldOfStudy.trim().length > 100) e.fieldOfStudy = "Field of study cannot exceed 100 characters.";

    if (!form.startYear) e.startYear = "Start year is required.";
    else if (form.startYear < 1950) e.startYear = "Start year cannot be before 1950.";
    else if (form.startYear > currentYear + 5) e.startYear = `Start year cannot exceed ${currentYear + 5}.`;

    if (form.endYear !== undefined && form.endYear !== null) {
      if (form.endYear < 1950) e.endYear = "End year cannot be before 1950.";
      else if (form.endYear > currentYear + 10) e.endYear = `End year cannot exceed ${currentYear + 10}.`;
      else if (form.endYear < form.startYear) e.endYear = "End year cannot be before start year.";
    }

    if (form.grade && form.grade.trim().length > 20) e.grade = "Grade cannot exceed 20 characters.";
      ...
      <DialogContent>
        <Stack spacing={3} sx={{ pt: 2 }}>
          ...
          <TextField
            label="Grade / CGPA"
            fullWidth
            placeholder="e.g. 8.7 CGPA, First Division"
            value={form.grade ?? ""}
            onChange={(e) => { f("grade", e.target.value); clearError("grade"); }}
            error={!!errors.grade}
            helperText={errors.grade || "Optional"}
