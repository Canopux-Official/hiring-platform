...
import {
  ...
  fetchPendingRecruiters, reviewRecruiter, IUser, // ✅ new imports
...
export default function AdminDashboard() {
  ...
  const statCards = [
    {
      ...
      icon: <PeopleAltIcon />,
      color: "#60a5fa",
      sub: stats ? `${stats.usersByRole.recruiter} recruiters · ${stats.usersByRole.job_seeker} seekers` : undefined,
    },
    {
      label: "Total Jobs",
      value: stats?.totalJobs ?? "–",
      icon: <WorkIcon />,
      color: "#34d399",
      sub: stats ? `${stats.jobsByStatus.open} open · ${stats.jobsByStatus.draft} draft` : undefined,
    },
    {
      label: "Applications",
      value: stats?.totalApplications ?? "–",
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      {/* ✅ Pending Recruiters section — shown only when there are pending requests */}
          ...
          {pendingLoading ? (
            ...
            <Stack spacing={1.5}>
              {pendingRecruiters.map((recruiter) => (
                ...
                >
                  <Avatar sx={{ bgcolor: alpha("#f59e0b", 0.2), color: "#f59e0b", fontWeight: 700, flexShrink: 0 }}>
