...
import { TableRowSkeleton } from "./Skeletons";
import { DeleteUserDialog } from "./DeleteUserDialog";
import { RecruiterDetailDrawer } from "./RecruiterDetailDrawer";
import { SeekerDetailDrawer } from "./SeekerDetailDrawer";
...
export function UsersTab({ onStatsRefresh }: UsersTabProps) {
  ...
  // â”€â”€â”€ Detail drawers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const [recruiterDrawer, setRecruiterDrawer] = useState<IUser | null>(null);
  const [seekerDrawer, setSeekerDrawer] = useState<IUser | null>(null);
  ...
  async function handleToggle(user: IUser) {
    ...
    try {
      ...
      toast.success(`${user.name} ${updated.isActive ? "activated" : "deactivated"}`);
      onStatsRefresh();
    } catch (err) {
      toast.error(getErrorMessage(err, "Failed to update user status."));
    } finally {
  ...
  // Opens the correct detail drawer based on role
  function handleRowClick(user: IUser) {
    const role = user.role.toLowerCase();
    if (role === "recruiter") setRecruiterDrawer(user);
    else if (role === "job_seeker") setSeekerDrawer(user);
    // admins: no drawer
  }

  const isClickable = (user: IUser) => {
    const role = user.role.toLowerCase();
  ...
  return (
    <Box>
        ...
        <Table size="small">
          ...
          <TableBody>
            {loading
              ...
              : (data?.items ?? []).map((user) => {
                  const roleColor = ROLE_COLORS[user.role] ?? "#94a3b8";
                  const clickable = isClickable(user);

                  return (
                    <TableRow
                      key={user._id}
                      onClick={() => clickable && handleRowClick(user)}
                      sx={{
                        "&:hover": { bgcolor: alpha("#ffffff", 0.025) },
                        opacity: user.isActive ? 1 : 0.55,
                    ...
                    >
                        ...
                        <Stack direction="row" spacing={1.5} alignItems="center">
                          ...
                          <Box sx={{ minWidth: 0 }}>
                            <Stack direction="row" spacing={0.5} alignItems="center">
                              ...
                              {/* Small hint icon for clickable roles */}
                              {clickable && (
                                <OpenInNewIcon
                      ...
                      <TableCell align="right">
                        <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                          <Tooltip title={user.isActive ? "Disable account" : "Enable account"}>
                            <span>
                              <IconButton
                                size="small"
      ...
      <RecruiterDetailDrawer
        recruiter={recruiterDrawer}
        onClose={() => setRecruiterDrawer(null)}
      />
      <SeekerDetailDrawer
        seeker={seekerDrawer}
        onClose={() => setSeekerDrawer(null)}
      />
    </Box>
  );
}
